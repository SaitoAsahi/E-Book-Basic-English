...
def main():
    ...
    print('Number of vocabularies included in the data set: ', len(word_set))
    print('Number of words in the dataset after error correction: ', len(correct_word_set))
    ...
    print('Created a dictionary type variable for {word:word ID}: {word:word ID}')
    #print(stoi)
    
    # 単語が値、単語IDがkeyの辞書を作成
    itos = dict(zip(ids, word_list))
    print('Created a dictionary type variable for {word ID:word}: {word ID:word}')
    #print(itos)
    ...
    print('Word dictionaries are now saved')

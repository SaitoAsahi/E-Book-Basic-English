...
class VisionModule(nn.Module):
    ...
    def forward(self, x):
        ...
        #print('visionモジュール特徴マップのサイズ', convnet(x).shape)
...
class CNN_CNN(nn.Module):
    ...
    def __init__(self, max_length=20, embedding=None, train_cnn=False):
        ...
        self.attention_module = AttentionModule(2048, 300)

        # Modulo de prediccion
        #self.prediction_module = PredictionModule(512, 300, self.vocab_size)
        self.prediction_module = PredictionModule(2048, 300, self.vocab_size)
...
class HierarchicalAttentionModule(nn.Module):
    ...
    def __init__(self, n_layers=6, kernel_size=3, embedding_dim=300, image_features=2048):
...
class CNN_CNN_CE(nn.Module):
    ...
    def sample(self, img, stoi, itos):
        ...
        for _ in range(self.max_length):
            ...
            for i, sentence in enumerate(sentences):
                ...
                # </s>の時にはキャプションに加えないように制限
                if itos[word_ids.cpu().numpy()[i]] != '</s>':
                    sentence.append(itos[word_ids.cpu().numpy()[i]])
               
            # 新しく生成された単語が</s>の場合は新しい単語の生成を止める
            if itos[word_ids.cpu().numpy()[i]] == '</s>':
                break
    ...
    def save(self, epoch):
        ...
        torch.save(dump, './weights/cnn_cnn_ce_{}_{}_{}_{}.dat'.format(self.vocab_size, self.embed_size, self.n_layers, epoch))
        

    def load(self, epoch):
        file = './weights/cnn_cnn_ce_{}_{}_{}_{}.dat'.format(self.vocab_size, self.embed_size, self.n_layers, epoch)
